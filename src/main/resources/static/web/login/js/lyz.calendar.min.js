/**
 * 修改源代码使之支持dialog的相对布局
 */
jQuery.fn.extend({
    calendar: function(c) {
    	
    		var winObj=c.parentWindow||$(document);
    		var parentObj=c.parentObj;
    		
        function r() {
        		
            winObj.find("#" + c.controlId).find(".tabD a").mouseup(function() {
                var a = new Date(winObj.find("#" + c.controlId).find(".currentYear").text() + "/" + winObj.find("#" + c.controlId).find(".currentMonth").text() + "/1");
                if ($(this).hasClass("prevD")) {
                    a.setMonth(a.getMonth() - 1);
                    a.setDate($(this).text());
                    var b = c.speed;
                    c.speed = 0;
                    winObj.find("#" + c.controlId).find(".prevMonth").triggerHandler("mouseup");
                    c.speed = b
                    ...
                    b = c.speed;
                    c.speed = 0;
                    winObj.find("#" + c.controlId).find(".nextMonth").triggerHandler("mouseup");
                    c.speed = b
                }
                var d = $(this).text();
                a = a.getFullYear() + "-" + (Number(a.getMonth() + 1) < 10 ? "0" + Number(a.getMonth() + 1) : Number(a.getMonth() + 1)) + "-" + (Number(d) < 10 ? "0" + d: d);
                winObj.find("#" + c.controlId).data("forObj").val(a);
                
                winObj.find("#" + c.controlId + " div table a").removeClass("select");
                winObj.find("#" + c.controlId + " .tabD a:contains('" + d + "')").each(function() {
                    d == $(this).text() && !$(this).hasClass("prevD") && !$(this).hasClass("nextD") && $(this).addClass("select")
                });
                winObj.find("#" + c.controlId).hide();
                c.callback()
            }).hover(function() {
                $(this).addClass("hover")
            },
        ...
        function u() {
            winObj.find("#" + c.controlId).find(".tabM a").mouseup(function() {
                var a = s(Number(winObj.find("#" + c.controlId).find(".currentYear").text()), Number($(this).attr("val")));
                D(a);
                r();
                winObj.find("#" + c.controlId).find(".currentMonth").text(Number($(this).attr("val")) + 1)
            }).hover(function() {
                $(this).addClass("hover")
            },
            function() {
        ...
        function v() {
            winObj.find("#" + c.controlId).find(".tabY a").mouseup(function() {
                var a = s(Number($(this).text()), Number(winObj.find("#" + c.controlId).find(".currentMonth").text()) - 1);
                D(a);
                r();
                winObj.find("#" + c.controlId).find(".currentYear").text(Number($(this).text()))
            }).hover(function() {
                $(this).addClass("hover")
            },
            function() {
        ...
        function B(a) {
            var b = winObj.find("#" + c.controlId).find(".reserve"),
            d = winObj.find("#" + c.controlId).find(".enabled");
        ...
        winObj.find("body").append(k);
        
        
        
        r();
        winObj.find("#" + c.controlId).find(".prevMonth").mouseup(function() {
            if (winObj.find("#" + c.controlId).find(".enabled > .tabD").length > 0) {
                var a = winObj.find("#" + c.controlId).find(".currentYear"),
                b = winObj.find("#" + c.controlId).find(".currentMonth"),
                ...
                r()
            } else if (winObj.find("#" + c.controlId).find(".enabled > .tabM").length > 0) {
                d = z(Number(winObj.find("#" + c.controlId).find(".currentYear").text()) - 1);
                C(d);
                u();
                winObj.find("#" + c.controlId).find(".currentYear").text(Number(winObj.find("#" + c.controlId).find(".currentYear").text()) - 1)
            } else if (winObj.find("#" + c.controlId).find(".enabled > .tabY").length > 0) {
                d = A(Number(winObj.find("#" + c.controlId).find(".currentYear").text()) - 10);
                C(d);
                v();
                winObj.find("#" + c.controlId).find(".currentYear").text(Number(winObj.find("#" + c.controlId).find(".currentYear").text()) - 10)
            }
        });
        winObj.find("#" + c.controlId).find(".nextMonth").mouseup(function() {
            if (winObj.find("#" + c.controlId).find(".enabled > .tabD").length > 0) {
                var a = winObj.find("#" + c.controlId).find(".currentYear"),
                b = winObj.find("#" + c.controlId).find(".currentMonth"),
        ...
        winObj.find("#" + c.controlId).find(".currentMonthText").mouseup(function() {
            if (! (winObj.find("#" + c.controlId).find(".enabled > .tabM").length > 0)) {
                var a = z(Number(winObj.find("#" + c.controlId).find(".currentYear").text()));
                E(a);
                u()
            }
        });
        winObj.find("#" + c.controlId).find(".currentYearText").mouseup(function() {
            if (! (winObj.find("#" + c.controlId).find(".enabled > .tabY").length > 0)) {
                var a = A(Number(winObj.find("#" + c.controlId).find(".currentYear").text()));
                E(a);
                v()
            }
        });
        n.bind("click focus",
        function() {
            if (winObj.find("#" + c.controlId + ":hidden").length != 0) {
                $(".calendar").hide();
                var a = winObj.find("#" + c.controlId),
                b = F(n[0]),
                ...
                d = 200;
                b = winObj.find("#" + c.controlId).height();
                a.width(0);
                a.height(0);
                
                var poi={"left":0,"top":0}&&parentObj.webpro.mainDialog.getDialogPosition();
                
                a.css({"left":poi.left+$(this).position().left,"top":poi.top+$(this).position().top+65});
                a.data("forObj",$(this));
        ...
        $(document).mouseup(function(a) {
            if ($(a.target).attr("id") != n.attr("id") && ($(a.target).parentsUntil("#" + c.controlId).parent().length == 0 || $(a.target).parentsUntil("#" + c.controlId).parent()[0].id != c.controlId)) winObj.find("#" + c.controlId).hide()
        })
    }
});