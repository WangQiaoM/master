...
WXUtils=function(wxCon,appName,baseUrl) {
    var wxApiList = ['checkJsApi', 'chooseWXPay', 'hideMenuItems','onMenuShareTimeline','chooseImage','uploadImage','downloadImage','getLocalImgData','onMenuShareAppMessage','onMenuShareQQ','onMenuShareWeibo','scanQRCode','getLocation','openLocation'];
    ...
    var E={
        ...
        listennerChooseImage:function (backFun) {
            wxCon.chooseImage({
                ...
                sourceType: ['camera'], // 可以指定来源是相册还是相机，默认二者都有 相册可加入,
                success: function (res) {
                   // var localIds = res.localIds; // 返回选定照片的本地ID列表，localId可以作为img标签的src属性显示图片
                    wxCon.getLocalImgData({
                        localId: res.localIds, // 图片的localID
                        success: function (res) {
                            //var localData = res.localData; // localData是图片的base64数据，可以用img标签显示
                            backFun(res.localData);
                        }
                    });
                }
            });
        ...
        //上传图片接口
        listennerUploadImage:function (backFun,localid) {
            wxCon.uploadImage({
        ...
        //下载图片接口
        listennerDownloadImage:function (backFun,serverid) {
            wxCon.downloadImage({
        ...
        //获取本地图片接口
        listennerGetLocalImgData:function (backFun,localid) {
            wxCon.getLocalImgData({
